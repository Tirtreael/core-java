...
//        LOGGER.info("Target0: " + target);
    ...
    public void setNewNonce() {
        if (this.sequentialNonce) {
            this.nonce++;
            byte[] nonceArray = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(nonce).array();
            this.headerBin[28] = nonceArray[0];
            this.headerBin[29] = nonceArray[1];
            this.headerBin[30] = nonceArray[2];
            this.headerBin[31] = nonceArray[3];
//            this.headerBin = ByteBuffer.allocate(32).order(ByteOrder.LITTLE_ENDIAN).put(28, Arrays.copyOfRange(nonceArray, 0,4)).array();
//            this.headerBin = ByteBuffer.allocate(32).order(ByteOrder.LITTLE_ENDIAN).put(ByteBuffer.allocate(28).order(ByteOrder.LITTLE_ENDIAN).putInt(nonce).array()).array();
        ...
        while (true) {
            int newNonce = new Random().nextInt(0, 0x7FFFFFFF);
